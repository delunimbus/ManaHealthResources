...
function spell:getResourceType(chara)
    if chara.id == "noelle" and chara:checkWeapon("thornring") then
        return "health"
    end
    return super.getResourceType(self, chara)
end
...
function spell:getMPCost(chara)
    local cost = chara:getStat("mana") * 2
    if chara and chara:checkWeapon("thornring") then
        cost = chara:getStat("mana")
    end
    return cost
end

function spell:isUsable(chara)
    if chara.id == "noelle" and chara:checkWeapon("thornring") and self:getResourceType(chara) == "health" then
        if chara:getHealth() > MathUtils.round(chara:getStat("health") / 3) then
            return false
        end
    end
    return super.isUsable(self, chara)
end

function spell:getHPCost(chara)
    local cost = chara:getStat("health") + 1
    if chara:getHealth() > chara:getStat("health") then
        cost = chara:getHealth() + 1
    end
    if chara and chara:checkWeapon("thornring") then
        cost = MathUtils.round(chara:getStat("health") / 3) - 1
    end
    return cost
end
